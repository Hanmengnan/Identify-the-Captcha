...
def Thresholding(picpath):
    ...
    if 'red' in str(picpath):
        ...
        for i in range(y):
            for j in range(x):
                if(imgArray[j, i][0] < 125 and imgArray[j, i][1] < 125 and imgArray[j, i][2] < 125 and (imgArray[j, i][0]*0.21267+imgArray[j, i][1]*0.71516+imgArray[j, i][2]*0.067) < 125):
                    img.putpixel(
                        (j, i), (0, 0, 0))
                else:
                    img.putpixel((j, i), (255, 255, 255))
...
def predict_all_letter(picpath):
    ...
    :return 预测字符
    """
    sdk = SDK(r"./model/Captcha-CNNX-NoRecurrent-H64-CrossEntropy-C3_model.yaml")
...
def get_aim_letters(captcha_json):
    """
    获取目标条件字符
    ...
    picpath = "./tempPic/"+captcha_type+"_"+str(uuid.uuid1())+".png"
    
    captcha_base64 = base64.b64decode(captcha_base64[22:])#去除前缀
